...
$('#saveExit').onclick=()=>{ localStorage.setItem('classlift-diagnostic-cn',JSON.stringify(state)); alert('进度已保存在当前浏览器。'); };
...
$('#leadForm').onsubmit=async e=>{
  ...
  try{
    ...
    localStorage.removeItem('classlift-diagnostic-cn');
...
function renderReport(serverResult){
 ...
 <section class="report-section"><span class="section-index">01 · YOUR DIRECTION</span><h3>你希望公司变成什么样？</h3><div class="goal-box">${ai?.desiredOutcomeSummary||`你目前最希望优先解决「${state.answers.primary_pain}」，让运营更加稳定并具备规模化基础。${state.answers.additional_needs?` 你还提到：${state.answers.additional_needs}`:''}`}<small>Based on your answers${ai?.aiGenerated?' · AI organized':''}</small></div></section>
